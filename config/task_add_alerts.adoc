...
+
.. Digite *abc* no campo *Nome contém* para exibir os volumes cujo nome contém "`abc`".
.. Selecione *+<<All Volumes whose name contains 'abc'>> +* da área Recursos Disponíveis e mova-o para a área Recursos Selecionados.
.. Clique em *Excluir* e insira *xyz* no campo *Nome contém* e depois clique em *Adicionar*.
...
. Clique em *Ações* e insira *sample@domain.com* no campo Alertar estes usuários.
