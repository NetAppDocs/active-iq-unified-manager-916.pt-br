---
permalink: config/concept_overview_of_configuration_sequence.html 
sidebar: sidebar 
keywords: overview, configuration, sequence 
summary: O fluxo de trabalho de configuração descreve as tarefas que você deve executar antes de poder usar o Unified Manager. 
---
= Visão geral da sequência de configuração
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
O fluxo de trabalho de configuração descreve as tarefas que você deve executar antes de poder usar o Unified Manager.

image::../media/post_install_flow.png[fluxo pós-instalação]
