---
permalink: config/concept_what_rbac_is.html 
sidebar: sidebar 
keywords: rbac, active iq unified manager, server 
summary: 'O RBAC (controle de acesso baseado em função) fornece a capacidade de controlar quem tem acesso a vários recursos e funcionalidades no servidor do Active IQ Unified Manager .' 
---
= O que é RBAC
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
O RBAC (controle de acesso baseado em função) fornece a capacidade de controlar quem tem acesso a vários recursos e funcionalidades no servidor do Active IQ Unified Manager .
